...
export async function compareJSONSchemas(oldSchema, newSchema) {
                ...
                if (typeof oldSubSchema === 'boolean') {
                    ...
                    if (oldSubSchema.maxLength) {
                        if (oldSubSchema.maxLength > newSubSchema?.maxLength) {
                            breakingChanges.push(`${path}/maxLength is changed from ${oldSubSchema.maxLength} to ${newSubSchema?.maxLength}`);
                        }
                    }
                    if (oldSubSchema.minLength) {
                        if (oldSubSchema.minLength < newSubSchema?.minLength) {
                            breakingChanges.push(`${path}/minLength is changed from ${oldSubSchema.minLength} to ${newSubSchema?.minLength}`);
                        }
                    }
                    if (oldSubSchema.pattern) {
                        if (newSubSchema?.pattern?.toString() !== oldSubSchema.pattern.toString()) {
                            breakingChanges.push(`${path}/pattern is changed from ${oldSubSchema.pattern} to ${newSubSchema?.pattern}`);
                        }
                    }
                    ...
                    if (oldSubSchema.type) {
                        if (typeof newSubSchema?.type === 'string'
